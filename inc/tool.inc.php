...
<body>
<div class="notice"><span class="pic {$pic}"></span>{$message} <a href="{$url}"> 3秒钟后自动跳转！</a> </div>
</body>
</html>
A;
    echo $html;
...
//验证前台用户是否登录
    ...
    function check_user($member_id,$conter_member_id,$is_manage_login){
        if($member_id == $conter_member_id ||$is_manage_login) {
            return true;
        }else{
            return false;
        }
